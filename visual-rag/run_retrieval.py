...
def extract_core_info_with_llm(question, model_name):
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": f"You are given a query, rephrase the query into a short descriptive phrase that highlights the key part of the entity where the queried feature could be found. DO NOT include the asked feature (shape, color, etc.) but instead include the part of the entity where the feature could be found. Output only the rephrased query.\n\nExample:\n\nOriginal query: What shape are the flowers of bush flax (scientific name: Astelia fragrans)?\nRephrased query: flowers of bush flax\n\nOriginal query: Is the any specific color pattern on underside wings of tawny pipit (scientific name: Anthus campestris) displayed during flight, or is it uniformly colored?\nRephrased query: tawny pipit with its underside wings shown\nOriginal query: {question}.\n\nRephrased query:",
            }
            ],
            max_tokens=300,
            temperature=0.0,
...
if __name__ == '__main__':
    ...
    output_dict = {'question': [], 'answer': [], 'num_pos_imgs': [], 'num_neg_imgs': [], 'retrieval_similarity': [], 'aggregated_score': [], 'ranked_imgs': []}
    ...
    with torch.no_grad():
        for index in tqdm(range(len(in_data))):
            ...
            if args.query_type == 'llm_extracted_text':
                question = extract_core_info_with_llm(cur_index_data['question'], args.query_expansion_model_name) 
                if args.embedding_model_name == 'clip':
                ...
                if args.query_type == 't2i_llm':
                    rephrased_query = extract_core_info_with_llm(cur_index_data['question'], args.query_expansion_model_name)
                else:
                    raise NotImplementedError
